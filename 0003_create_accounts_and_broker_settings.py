...
def upgrade() -> None:
    ...
    """)
    op.execute('CREATE INDEX IF NOT EXISTS idx_broker_settings_key ON broker_settings (key);')

    # 7. broker_settings updatedAt trigger
    op.execute('DROP TRIGGER IF EXISTS trg_update_broker_settings_updated_at ON broker_settings;')
    op.execute("""
