"""merge heads + composite (market_type, gateway, account_id) key on accounts,
add market_type/gateway to trades
...
- Adds ``market_type``/``gateway`` to ``trades`` (nullable, backfilled from the
...
- Replaces ``accounts``' single-column unique constraint with a composite one
  on ``(market_type, gateway, account_id)``.
...
Until the worker side disambiguates by ``market_type``+``gateway`` too, avoid
...
def upgrade() -> None:
  # 1. Add + backfill trades.market_type/gateway while accounts.account_id is
  ...
  op.add_column(
    "trades",
    sa.Column(
      "market_type", PgEnum(name="markettypeenum", create_type=False), nullable=True
    ),
  )
  ...
  op.execute("""
        ...
        WHERE t.account_id = a.account_id
          AND t.market_type IS NULL;
    """)

  # 2. trades: (account_id, ref_id) -> (market_type, gateway, account_id, ref_id)
